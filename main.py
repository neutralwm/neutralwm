...
def new_borders():
	if double_borders:
		subprocess.call("sleep 0.2; chwb -s 8 -c 0xafafaf $(xwininfo -tree -root | grep urxvt | cut -c 6-14)", shell=True)
		subprocess.call("chwb2 -i 1 -o 9 -I " + border_colour + " -O $(xrdb -query | head -n 2 | tail -n 1 | cut -c 16-) $(xwininfo -tree -root | grep urxvt | cut -c 6-14)", shell=True)
	if single_border:
		subprocess.call("sleep 0.2; chwb -s 8 -c 0x" + border_colour + " $(lsw)", shell=True)
